...
This script builds `Rdata` annual databases.  See `Build Database.Rmd` for the consolidation of these database into two databases that span years. 

#### _ISSUES_
1. 

#### _TODO_
1.  Add the number of days for which the average is taken
2.  churn: run for each year
...
suspect_stations_df <- data_sum_hour %>%
  mutate(suspect_station = ifelse(max_flow > 250 & min_flow == 0L, TRUE, FALSE)) %>%
  select(station, hour, district, route, type, direction, suspect_station)

data_clean_suspect <- left_join(data_clean, suspect_stations_df, by = c("station", "hour", "district", "route", "type", "direction"))
...
data_sum_period <- data_clean_suspect %>%
  ...
  summarise(flow = sum(flow), speed_flow = sum(speed_flow), occup_flow = sum(occup_flow)) %>%
